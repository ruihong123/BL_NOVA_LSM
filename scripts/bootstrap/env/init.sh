...
for ((i=0;i<numServers;i++)); do
	...
	echo "*******************************************"
   	...
   	echo "*******************************************"
   	echo "*******************************************"
#   	&& git clone https://github.com/ruihong123/BL_NOVA_LSM
 	ssh -oStrictHostKeyChecking=no node-$i "sudo apt-get update  && cd BL_NOVA_LSM && bash scripts/bootstrap/env/install-deps.sh && mkdir build && cd build && cmake .. && make db_bench nova_server_main"
