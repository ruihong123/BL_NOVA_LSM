...
results="/tmp/results"
recordcount="$1"
exp_results_dir="$home_dir/high-load-nova-lsm-sr-offload-$recordcount"
dryrun="$2"
...
function run_bench() {
	...
	result_dir_name="nova-d-$dist-w-$workload-ltc-$number_of_ltcs-stoc-$nstoc-l0-$l0_stop_write_mb-np-$num_memtable_partitions-ss-$sstable_size_mb-p-$ltc_num_stocs_scatter_data_blocks-sp-$scatter_policy-mc-$major_compaction_type"
...
for workload in "workloadw" "workloada"
do
for dist in "uniform" #"zipfian"
do
for major_compaction_type in "lc"
do
run_bench
done
done
...
for workload in "workloade" #"workloada"
do
for dist in "uniform" "zipfian"
do
for major_compaction_type in "lc" "sc"
do
run_bench
done
done
